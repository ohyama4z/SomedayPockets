...
### タスクの進め方
1. **着手**: `in-progress` ラベルを付与し、Issueから作業ブランチを切る（`issue/番号-簡潔な名前`）。Issueにコメントで作業計画を書く（チェックリストでサブタスク分解）
2. **作業中**: 意味のあるまとまりごとにコミット・pushする。判断・方針変更・ブロッカーが発生したらIssueにコメントで記録する。サブタスク完了時はIssue本文のチェックリストを更新する
3. **完了**: PRを作成し（本文に `Closes #番号` を含める）、mainにマージする。マージによりIssueが自動クローズされる
