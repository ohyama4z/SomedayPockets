...
- 変数名・関数名などの識別子は英語
- ユーザーへの問いかけ・確認は原則 `AskUserQuestion` のselection形式で行う（タップだけで回答できるようにする）

## ドキュメント運用

### ストック情報（時間が経つほど価値が高まる）
- 仕様は `docs/spec/` に機能別ファイルで管理する。コードからぱっと拾えない情報や機能概要のサマリを記載する
- 技術的な意思決定は `docs/decisions/` にADR形式で残す（命名: `NNN-タイトル.md`、テンプレート: `docs/decisions/_template.md`）。ADRを書く際はAskUserQuestionで採用/不採用を確認する
- 開発知見は `knowledge/` に1知見1ファイルで管理する（1ファイル＝独立して参照・検索できる1つの学び。複数の学びは連番を分けて別ファイルにする。命名: `日付_チケット番号_連番_タイトル.md`、例: `2026-03-10_T11_1_docs運用.md`）。詳細は `docs/decisions/005-knowledge_1知見1ファイルルールを採用する.md` を参照。知見の読み書きはAIが自律的に行い、ユーザーへの確認は不要
...
## 定期レビュー
- セッション中は `/loop 10m /review-process` で定期的にプロセスレビューを実行する
