...
## 位置付け
セッション開始時のタスク状態（進行中タスク・オープンなIssue・ブランチ・未コミット変更・直近コミット）はSessionStartフック（`.claude/scripts/session-start.sh`）が自動出力する。本スキルはその補完であり、フック出力で情報が不足する場合（in-progressタスクのコメント深掘り・知見の読み込みなど）の手動実行用として使う。
